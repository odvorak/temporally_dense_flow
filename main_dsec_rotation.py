...
import models
from augmented_datasets_rotation import AugmentedDSECDatasetRotation
...
class CustomTransform:
    def __call__(self,x):
        self.angle = torch.rand(1) * 360
        rotated = transforms.functional.rotate(x.unsqueeze(0), self.angle[0].detach().item())
        cropped = transforms.functional.center_crop(rotated, (128, 128))
        return cropped.squeeze(0), self.angle
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        train_dataset = AugmentedDSECDatasetRotation(dataset_dir, dt=args.dt, n_split=args.n_split,
        ...
        test_dataset = AugmentedDSECDatasetRotation(dataset_dir, dt=args.dt, n_split=args.n_split,
