...
class AdaptiveFlowNet2S(BaseModel):
    def __init__(self, model_options):
        ...
        self.residual11 = general_conv2d(in_channels=2*self._BASE_CHANNELS, out_channels=2*self._BASE_CHANNELS, kernel_size=3, stride=1, padding=1,  
                                        batchNorm=self.batchNorm, model_options=model_options)
        self.residual12 = general_conv2d(in_channels=2*self._BASE_CHANNELS, out_channels=2*self._BASE_CHANNELS, kernel_size=3, stride=1, padding=1,  
                                        batchNorm=self.batchNorm, model_options=model_options)
        self.residual21 = general_conv2d(in_channels=2*self._BASE_CHANNELS, out_channels=2*self._BASE_CHANNELS, kernel_size=3, stride=1, padding=1,  
                                        batchNorm=self.batchNorm, model_options=model_options)                                
        self.residual22 = general_conv2d(in_channels=2*self._BASE_CHANNELS, out_channels=2*self._BASE_CHANNELS, kernel_size=3, stride=1, padding=1,  
                                        batchNorm=self.batchNorm, model_options=model_options)

        self.deconv1 = nn.Sequential(
            nn.ConvTranspose2d(2*self._BASE_CHANNELS, 4, kernel_size=4, stride=2, padding=1, bias=False),
        ...
        self.upsampled_flow2_to_1 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=2*self._BASE_CHANNELS, out_channels=32, kernel_size=4, stride=2, padding=1, bias=False),
            LIF(leak=model_options['ileak'], threshold=model_options['ithresh'], learn_leak=model_options['learn_leak'], learn_threshold=model_options['learn_thresh'],
                reset_mechanism=model_options['reset_mechanism'], per_channel=model_options['per_channel'])
        )
    ...
    def forward(self, event_voxel):
        ...
        for i in range(event_reprs.size(4)):
            ...
            #RESIDUAL
            #Block-1
            out_r11 = self.residual11(out_enc2)
            out_r12 = self.residual12(out_r11) + out_enc2
            ...
            #DECODER
            flow2 = self.predict_flow2(self.upsampled_flow2_to_1(out_r22))
            flow2_acc = self.predict_flow2_acc(flow2)
            out_dec1 = self.deconv1(out_r22)
...
if __name__ == '__main__':
    ...
    print('--- Test Model ---')
    inps = torch.rand([1, 20, 2, 128, 128])  # (B, T, C, H, W)
